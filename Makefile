...
# Use gotestsum in CI for human-readable test output; use plain go test locally
ifdef CI
GO_TEST_EXEC ?= gotestsum --format=testdox --
else
...
# `cmd/pulumi-resource-minio/schema.json` is valid and up to date.
